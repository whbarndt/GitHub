...
local_config_yaml_file_path = 'config.local.finance.yml'

# Open and read the YAML files
with open(local_config_yaml_file_path, 'r') as file:
    local_config_yaml = yaml.safe_load(file)

config_yaml_file_path = local_config_yaml['Working_Dir'] + 'config.finance.yml'

with open(config_yaml_file_path, 'r') as file:
    config_yaml = yaml.safe_load(file)

businesses_yaml_file_string = 'businesses.finance.yml'
businesses_yaml_file_path = local_config_yaml['Working_Dir'] + businesses_yaml_file_string

with open(businesses_yaml_file_path, 'r') as file:
    businesses_yaml = yaml.safe_load(file)

# Load data from config file
year = local_config_yaml['Year']
path_to_expense_dir = local_config_yaml['Working_Dir']
